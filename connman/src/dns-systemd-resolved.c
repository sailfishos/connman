...
static bool skip_iface_update(int index)
{
	...
	/*
	 * If the default service exists and it is different do not allow to
	 * update this service. Only the default service's DNS should be used.
	 * The services may be updated after the service was disabled but that
	 * would then block use of the service at startup as DNSs and domains
	 * are updated before the service is enabled. This makes it possible
	 * to handle both cases, when the service is just initializing or when
	 * a service gets updates while not being as default service, which is
	 * the case when using VPN as default service.
	 */
	return default_service &&
			__connman_service_get_index(default_service) != index;
}
							...
							const char *server)
{
	...
	if (domain) {
		...
		iface->needs_domain_update = skip_update ? FALSE : TRUE;
	}

	if (server) {
		iface->servers = replace_to_end(iface->servers, server);
		iface->needs_server_update = skip_update ? FALSE: TRUE;
	}
